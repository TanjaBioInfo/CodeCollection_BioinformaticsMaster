...
# Rekonstruieren Sie Bäume mit dem UPGMA-Algorithmus und den Distanzmatrizen, die Sie oben berechnet haben
# Erstellen Sie eine Funktion zum Berechnen eines UPGMA-Baums aus einer gegebenen Matrix mit folgenden Eingaben
# inputmatrix: Matrix mit Distanzen zwischen Knoten im Baum
# internalnodestart: Halten Sie den Zähler für die internen Knoten, er beginnt bei 2n-1
# nodecounts: ist ein Vektor, der die Anzahl der Spitzen unter jedem Knoten mit einem
# entsprechenden Eintrag in der Distanzmatrix enthält
# edge: ist eine Matrix, die die Verbindungen (Kanten) zwischen den Knoten enthält, dies
# wird die Ausgabe der rekursiven Funktion sein
# edge_length: ist ein Vektor, der die Länge der in der „edge“-Matrix definierten Kanten definiert
# es ist nicht mit Distanz implementiert, das könnte die nächste Aufgabe sein
